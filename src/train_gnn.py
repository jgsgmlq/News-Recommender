"""
Training script for GNN-Enhanced News Recommendation Model
"""
import os
import sys
import argparse
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch, gnn_embeddings=None):
    """Train for one epoch"""
    model.train()
    total_loss = 0
    correct = 0
...
def validate(model, val_loader, criterion, device, gnn_embeddings=None):
    """Validate the model"""
    model.eval()
    total_loss = 0
    correct = 0
...
def main(args):
    ...
    # Create model
    print("\n" + "=" * 60)
    print("Creating GNN-Enhanced Model")
    print("=" * 60)
    ...
    # Tensorboard writer
    output_dir = os.path.join(args.output_dir, 'gnn' if args.use_gnn else 'baseline')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Pre-compute GNN embeddings (once per epoch to save computation)
        gnn_embeddings = None
        if args.use_gnn:
        ...
        train_loss, train_acc = train_epoch(
            model, train_loader, criterion, optimizer, device, epoch, gnn_embeddings
        )
        print(f"Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.2f}%")

        # Validate
        val_loss, val_acc = validate(model, val_loader, criterion, device, gnn_embeddings)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train GNN-Enhanced News Recommendation Model')
